            ...
            if (checkInput(x1, x2, y1, y2)) {
                ...
                if (GOOSE_TURN == true) {
                    if (gameBoard.getBoard()[x1][y1] == gameBoard.getGOOSE()) {
                        ...
                        findFox();
                        for (int i = xFox - 2; i <= Math.min(xFox + 2, gameBoard.getMaxIndex()); i++) {
                            for (int j = yFox - 2; j <= Math.min(yFox + 2, gameBoard.getMaxIndex()); j++) {
        ...
        switch (validMove) {
            ...
            case 1:
                if (gameBoard.getBoard()[x2][y2] == gameBoard.getFREE()) {
                    ...
                    //at this point the board is printed correctly
                    //IT WORKS
                    GOOSE_TURN = true;
                }
                else{
